...
def process_all(dir_in, dir_out, classifier):
...
def process(args):
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    classifier = EncoderClassifier.from_hparams(source=args.speaker_embed, run_opts={"device": device}, savedir=os.path.join('/tmp', "speechbrain/spkrec-xvect-voxceleb"))
